...
def search():
    ...
    # q is submitted to /search as a GET paramter
    q = request.args.get("q") + "%"

    # output JSON array of ojbects, each object is a row from places
    place = db.execute("SELECT * FROM places WHERE place_name LIKE :q OR postal_code LIKE :q OR admin_name1 LIKE :q", q=q)

    # Return up to 10 places to search
    if len(place) > 10:
